...
### 7. 🎯 自定义短语独立展示
- **独立区域**：在关键词榜单下方单独显示用户自定义的短语
- **实时统计**：自动计算每个自定义短语在标题中的出现次数
- **双击筛选**：双击自定义短语，右侧视频列表会自动筛选包含该短语的视频
- **渐变配色**：使用蓝粉渐变色区分自定义短语区域，视觉更加清晰

### 8. 🔄 重新加载数据
- **一键刷新**：点击模态框底部的"重新加载"按钮
- **实时更新**：重新从B站API获取最新的稍后再看和历史记录数据
- **完整爬取**：支持多次加载，确保获取尽可能全面的数据
- **自动分析**：重新加载数据后自动重新分析并更新UI显示
