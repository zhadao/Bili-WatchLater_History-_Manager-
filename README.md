# Bilibili 稍后再看 & 历史记录增强助手 (Bili-WatchLater_History-_Manager-)
...
作为一名 B 站网页版的重度使用者，我发现现有的 AI 浏览器（截至 2026 年1月中旬）在处理 B 站“稍后再看”列表时存在功能盲区，无法进行有效的总结。同时，B 站原生的“稍后再看”播放器存在诸多不便：**不方便查看完整评论区、难以切换分 P、互动（三连）体验割裂**。
...
*   **持久化**：你的屏蔽词和自定义词库会自动保存，刷新页面后依然有效。
