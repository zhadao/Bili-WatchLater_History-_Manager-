      ...
      if (phraseSet.size > 0) {
        phraseSet.forEach(phrase => {
          ...
          if (this.isCaseSensitive) {
            ...
            matches = processedTitle.match(regex);
          } else {
            // 合并模式：使用智能正则忽略大小写匹配
          ...
          if (matches) {
            ...
            // 替换为空格（使用智能正则）
            const replaceRegex = this.isCaseSensitive 
              ? this.createSmartRegex(phrase, 'g')
              : this.createSmartRegex(phrase, 'gi');
            processedTitle = processedTitle.replace(replaceRegex, ' ');
          }
        });
      }
